...
class LatestReleaseMonitor extends Action_1.Action {
	...
	async update(quality) {
		...
		try {
			lastKnown = await (0, blobStorage_1.downloadBlobText)(
				"latest-" + quality,
				"latest-releases",
				storageKey
			);
		...
		const latest =
			(_a = await (0, utils_1.loadLatestRelease)(quality)) === null ||
			_a === void 0
				? void 0
				: _a.version;
		if (latest && latest !== lastKnown) {
			(0, utils_1.safeLog)("found a new release of", quality);
			await (0, blobStorage_1.uploadBlobText)(
				"latest-" + quality,
				latest,
				"latest-releases",
				storageKey
			);
			await new octokit_1.OctoKit(token, github_1.context.repo).dispatch(
				"released-" + quality
			);
		}
	}
