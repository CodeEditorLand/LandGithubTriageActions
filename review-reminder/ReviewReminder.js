...
class ReviewReminder {
		...
		for await (const response of octokit.paginate.iterator(
			...
			for (const pr of response.data) {
				...
				for (const review of reviews.data) {
					...
					if (
						((_c = review.user) === null || _c === void 0
							? void 0
							: _c.login) ===
						((_d = pr.user) === null || _d === void 0
							? void 0
							: _d.login)
					) {
						continue;
					...
					if (ts < first.submitted_ts) {
						...
						first.reviewer =
							(_h =
								(_g = review.user) === null || _g === void 0
									? void 0
									: _g.login) !== null && _h !== void 0
								? _h
								: "MISSING";
					}
				}
	...
	async processAllRepositories(teamMembers) {
		...
		for (const review of data) {
			const monthlyCount =
				(_a = monthlyStats.get(review.reviewer)) !== null &&
				_a !== void 0
					? _a
					: 0;
			...
			if (diffDays < 7) {
				const weeklyCount =
					(_b = weeklyStats.get(review.reviewer)) !== null &&
					_b !== void 0
						? _b
						: 0;
		...
		for (const reviewer of bottomReviewers) {
			bottomReviewerStats.push({
				reviewer,
				monthlyCount:
					(_c = monthlyStats.get(reviewer)) !== null && _c !== void 0
						? _c
						: 0,
				weeklyCount:
					(_d = weeklyStats.get(reviewer)) !== null && _d !== void 0
						? _d
						: 0,
			});
		}
		...
		for (let i = 0; i < 3; i++) {
			...
			topReviewerStats.push({
				reviewer,
				monthlyCount:
					(_e = monthlyStats.get(reviewer)) !== null && _e !== void 0
						? _e
						: 0,
				weeklyCount:
					(_f = weeklyStats.get(reviewer)) !== null && _f !== void 0
						? _f
						: 0,
