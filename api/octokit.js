...
class OctoKit {
	...
	octokitIssueToIssue(issue) {
		...
		return {
			author: {
				name:
					(_b =
						(_a = issue.user) === null || _a === void 0
							? void 0
							: _a.login) !== null && _b !== void 0
						? _b
						: "unkown",
				isGitHubApp:
					((_c = issue.user) === null || _c === void 0
						? void 0
						: _c.type) === "Bot",
			},
			...
			number: issue.number,
			title: issue.title,
			isPr: !!((_e = issue.pull_request) === null || _e === void 0
				? void 0
				: _e.html_url),
			...
			assignee:
				(_g =
					(_f = issue.assignee) === null || _f === void 0
						? void 0
			...
			assignees:
				(_l =
					(_k = issue.assignees) === null || _k === void 0
						? void 0
						: _k.map((assignee) => assignee.login)) !== null &&
				_l !== void 0
					? _l
					: [],
	...
	octokitMilestoneToMilestone(milestone) {
		let _a;
		if (
			(milestone === null || milestone === void 0
				? void 0
		...
		return {
			...
			createdAt:
				milestone.created_at !== null
					? new Date(milestone.created_at.split("T")[0])
					: null,
			dueOn:
				milestone.due_on !== null
					? new Date(milestone.due_on.split("T")[0])
					: null,
			closedAt:
				milestone.closed_at !== null
					? new Date(milestone.closed_at.split("T")[0])
					: null,
			description:
				(_a = milestone.description) !== null && _a !== void 0
					? _a
					: "",
	...
	async readConfig(path) {
		...
		try {
			...
			throw Error(
				`Found directory at config path when expecting file${JSON.stringify(
					data,
				)}`,
			);
		} catch (e) {
			throw Error(
				`Error with config file at ${repoPath}: ${JSON.stringify(e)}`,
			);
		}
	}
...
class OctoKitIssue extends OctoKit {
	...
	async getAssigner(assignee) {
		...
		if (!assigner) {
			throw Error(
				`Expected to find ${assignee} in issue timeline but did not.`,
			);
		}
		return assigner;
	}
