...
describe("TestPlanItemValidator", () => {
	it("does nothing for valid test plan items", async () => {
		const testbed = new TestbedIssue(
			{},
			{ issue: { body: validTestPlanItem }, labels: ["tpi"] }
		);
	...
	it("does nothing for non test plan items", async () => {
		const testbed = new TestbedIssue(
			{},
			{ issue: { body: invalidTestPlanItem } }
		);
		await new TestPlanItemValidator(
			testbed,
			"",
	...
	it("adds comment for invalid test plan items", async () => {
		const testbed = new TestbedIssue(
			{},
			{ issue: { body: invalidTestPlanItem }, labels: ["tpi"] }
		);
		await new TestPlanItemValidator(
			testbed,
			"",
		...
		expect(
			comments[0].body.indexOf("Test plan item should have header") !== 1
		).to.be.true;
