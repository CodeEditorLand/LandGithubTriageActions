...
class FetchIssues extends Action_1.Action {
	...
	async onTriggered(github) {
		...
		for await (const page of github.query({ q: query })) {
			for (const issue of page) {
				...
				if (issueData.author.name === "ghost") {
					(0, utils_1.safeLog)(
						`Tagging issue  #${issueData.number} as invalid`,
					);
					try {
		...
		(0, child_process_1.execSync)(
			`unzip -q ${(0, path_1.join)(blobStorage, "area_model.zip")} -d ${(0, path_1.join)(models, "area_model")}`,
		);
		(0, utils_1.safeLog)("unzipping assignee model");
		(0, child_process_1.execSync)(
			`unzip -q ${(0, path_1.join)(blobStorage, "assignee_model.zip")} -d ${(0, path_1.join)(models, "assignee_model")}`,
		);
	}
}
