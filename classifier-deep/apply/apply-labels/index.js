	...
	async onTriggered(github) {
		...
		for (const labeling of labelings) {
			...
			const addAssignee = async (assignee) => {
				var _a;
				if (
					(_a = config.vacation) === null || _a === void 0
						? void 0
						: _a.includes(assignee)
				) {
					(0, utils_1.safeLog)(
						"not assigning ",
						assignee,
			...
			if (issueData.labels.includes("invalid")) {
				(0, utils_1.safeLog)(
					`issue ${labeling.number} is invalid, skipping`,
				);
				continue;
			}
			...
			);
			{
				...
				if (debug) {
					...
					await issue.postComment(
						`confidence for label ${category}: ${confidence}. ${confident ? "does" : "does not"} meet threshold`,
					);
				}
				if (confident) {
					(0, utils_1.safeLog)(
			...
			}
			{
				...
				if (debug) {
					...
					await issue.postComment(
						`confidence for assignee ${category}: ${confidence}. ${confident ? "does" : "does not"} meet threshold`,
					);
				}
				if (confident) {
