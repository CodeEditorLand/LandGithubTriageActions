...
class CodeReviewChatAction extends Action_1.Action {
	...
	async closedOrDraftHandler(_issue, payload) {
		...
		await new CodeReviewChat_1.CodeReviewChatDeleter(
			slackToken,
			elevatedUserToken,
			channelId,
			payload.pull_request.html_url,
	...
	async executeCodeReviewChat(github, issue, payload, external) {
		...
		return new CodeReviewChat_1.CodeReviewChat(
			github,
			new vscodeTools_1.VSCodeToolsAPIManager(),
			issue,
			{
				slackToken,
				...
				payload: {
					...
					repo_full_name:
						(_a = payload.repository.full_name) !== null &&
						_a !== void 0
							? _a
							: payload.repository.name,
					// https://docs.github.com/en/developers/webhooks-and-events/webhooks/webhook-events-and-payloads#pull_request
					pr: (0, CodeReviewChat_1.createPRObject)(
						payload.pull_request,
					),
				},
			},
	...
	async onSubmitReview(issue, payload) {
		...
		const auth = await this.getToken();
		const github = new rest_1.Octokit({ auth });
		const meetsThreshold = await (0, CodeReviewChat_1.meetsReviewThreshold)(
			github,
			teamMembers,
			payload.pull_request.number,
			payload.repository.name,
		...
		if (
			...
			((_a = payload.pull_request.user) === null || _a === void 0
				? void 0
				: _a.type) !== "Bot"
		) {
			(0, utils_1.safeLog)(
			...
			const teamMemberReviews = await (0,
			CodeReviewChat_1.getTeamMemberReviews)(
				github,
				teamMembers,
				payload.pull_request.number,
				payload.repository.name,
			...
			(0, utils_1.safeLog)(
				`Found ${(_b = teamMemberReviews === null || teamMemberReviews === void 0 ? void 0 : teamMemberReviews.length) !== null && _b !== void 0 ? _b : 0} reviews from team members`,
			);
			...
			const approvingReviews =
				teamMemberReviews === null || teamMemberReviews === void 0
					...
					: teamMemberReviews.filter((review) => {
							var _a;
							(0, utils_1.safeLog)(
								`Reviewer: ${(_a = review === null || review === void 0 ? void 0 : review.user) === null || _a === void 0 ? void 0 : _a.login} - ${review.state}`,
							);
	...
	async onTriggered() {
		...
		const pull_request = JSON.parse(
			(0, utils_1.getRequiredInput)("pull_request"),
		);
